...
export default function CandidateDetail() {
  ...
  // -------------------------------
  // CONFIRM VOTE (Sweet Alert)
  // -------------------------------
  const confirmVote = () => {
    Dialog.show({
      type: ALERT_TYPE.INFO,
      title: "Confirm Vote",
      textBody: `Are you sure you want to vote for ${candidate.name}?`,
      button: "Vote",
      onPressButton: () => {
        Dialog.hide();
        handleVote();
  ...
  const handleVote = async () => {
    if (alreadyVotedCategory) {
      return Dialog.show({
        ...
        title: "Error",
        textBody: `You already voted for ${category}.`,
        button: "OK",
      });
    }
    ...
    if (alreadyVotedCandidate) {
      return Dialog.show({
        type: ALERT_TYPE.DANGER,
        title: "Error",
        textBody: "You already voted for this candidate.",
        button: "OK",
      });
    }
    ...
    try {
      ...
      setTimeout(() => {
        Toast.show({
          ...
          title: "Success",
          textBody: "Vote submitted successfully!",
        });
      }, 300);
    } catch (err) {
      Dialog.show({
  ...
  if (!candidate)
    return (
      <Text style={{ textAlign: "center", marginTop: 40 }}>Not Found</Text>
    );
